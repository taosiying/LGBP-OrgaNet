...
class CNNPatchMerging(nn.Module):
...
#
#     def forward(self, x):
#         x = torch.cat((self.drop_path(self.attn(self.norm1(x))) + x[:, 0:1, ...], x[:, 1:, ...]), dim=1)
#         if self.has_mlp:
#             x = x + self.drop_path(self.mlp(self.norm2(x)))
#
#         return x
...
#                  drop_path=[0,0,0], act_layer=nn.GELU, norm_layer=nn.LayerNorm):
#         super().__init__()
#
#         num_branches = len(dim)
#         self.num_branches = num_branches
...
#             self.norms.append(nn.LayerNorm(dim[i],device='cuda'))
#         self.projs = nn.ModuleList()
#         for d in range(num_branches):
#             tmp = [nn.LayerNorm(dim[d]), act_layer(), nn.Linear(dim[d], dim[(d+2)%num_branches])]
#             self.projs.append(nn.Sequential(*tmp))
#         num_patches = [3136,784,196]
#         self.pos_embed = nn.ParameterList(
#             [nn.Parameter(torch.zeros(1, 1 + num_patches[i], dim[i])) for i in range(self.num_branches)])
#         self.fusion = nn.ModuleList()
#         for d in range(num_branches):
#             # d_ = (d + 1) % num_branches #1,2,0
#             d_ = d
#             nh = num_heads[d_]#3,3,3
#             if depth[-1] == 0:  # backward capability:
#                 self.fusion.append(
#                     CrossAttentionBlock(dim=dim[d_], num_heads=nh, mlp_ratio=mlp_ratio[d], qkv_bias=qkv_bias,
#                                         qk_scale=qk_scale,
...
#         for i in range(self.num_branches):
#             temp = x[i]
#             cls = self.average(self.norms[i](temp).transpose(1,2))
#             cls = Rearrange('b c 1 -> b 1 c')(cls)
#             temp_cls = torch.cat((cls,temp),dim=1)
#             x[i] = (temp_cls + self.pos_embed[i]).clone()
#         inp = x
#         # only take the cls token out
#         proj_cls_token = [proj(x[:, 0:1]) for x, proj in zip(inp, self.projs)]
#         # cross attention
#         outs = []
#         for i in range(self.num_branches):
#             tmp = torch.cat((proj_cls_token[(i + 1) % self.num_branches], inp[i][:, 1:, ...]), dim=1)
#             tmp = self.fusion[i](tmp)
#             outs.append(tmp[:,1:,...])
#         return outs
...
class CrossfusionBlock(nn.Module):
    def __init__(self,dim,L):
        super().__init__()
        self.num_heads = 3
        self.patchexpand = PatchExpand(dim=dim[1],dim_scale=2,norm_layer=nn.LayerNorm)
...
#     def __init__(self,dim = [96,192,384]):
#         super().__init__()
#         self.corssattention = MultiScaleBlock(dim=dim, num_heads=3,
#                                   mlp_ratio=[1,1,1],
#                                   qkv_bias=True, qk_scale=None, drop=0.,
#                                   attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm)
#     def forward(self,x):
#         x = self.corssattention(x)
#         p1,p2,p3 = x[0],x[1],x[2]
#         p2_p3_cat = torch.cat((p2,p3),dim=1)
...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    with torch.no_grad():
        ...
        out = torch.reshape(out, (1, patch_size[0], patch_size[1]))
    ...
    #     prediction = 255 * prediction.reshape((patch_size[0], patch_size[1])).astype(np.uint8)
    #     contours, hierarchy = cv2.findContours(prediction, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    #     )
    #     contour_image = inv_normalize(image)
    #     contour_image = contour_image.cpu().detach().numpy().astype(np.uint8)
    #     print('\ncontour_img:', contour_image.shape)
    #     contour_image = contour_image.reshape(224,224,3)
    #     cv2.imwrite('predictions/hiformer-b/1.png', contour_image)
    #     cv2.drawContours(contour_image, contours, -1, (255, 255, 255), 2)
    #     cv2.imwrite(test_save_path, contour_image)
        # edges = cv2.Canny(prediction.astype(np.uint8), 0, 1)
        # print('edges_shape',edges.shape)
        # result_image = image.cpu().detach().numpy()
    ...
    #             else:
    #                 pred = out
    #             print('prediction:',prediction[ind].shape,pred.shape)
    #             prediction[ind] = pred
    # else:
